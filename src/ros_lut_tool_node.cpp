...
void mouse_call(int event, int x, int y, int flags, void *data)
{
    if (event == cv::EVENT_LBUTTONDBLCLK & !lut_tool->mouse_stuff.left_down)
    {
        if (x >= frame_width & y < frame_height)
        {
            ...
            lut_tool->mouse_stuff.coor1.x = std::max(std::min(x, frame_width * 2 - 1), frame_width);
            lut_tool->mouse_stuff.coor1.y = std::max(std::min(y, frame_height - 1), 0);

            std::cout << "[lut_tool] Corner 1: " << lut_tool->mouse_stuff.coor1 << std::endl;
        }
        else
        {
    ...
    if (event == cv::EVENT_LBUTTONDBLCLK & lut_tool->mouse_stuff.left_down)
    {
        if (std::abs(x - lut_tool->mouse_stuff.coor1.x) > 1 & std::abs(y - lut_tool->mouse_stuff.coor1.y) > 1)
        {
            lut_tool->mouse_stuff.left_up = true;

            lut_tool->mouse_stuff.coor2.x = std::max(std::min(x, frame_width * 2 - 1), frame_width);
            lut_tool->mouse_stuff.coor2.y = std::max(std::min(y, frame_height - 1), 0);

            std::cout << "[lut_tool] Corner 2: " << lut_tool->mouse_stuff.coor2 << std::endl;
        }
        else
        {
    ...
    if (lut_tool->mouse_stuff.left_down & lut_tool->mouse_stuff.left_up)
    {
        ...
        cv::Rect box;
        box.width = std::abs(lut_tool->mouse_stuff.coor1.x - lut_tool->mouse_stuff.coor2.x);
        box.height = std::abs(lut_tool->mouse_stuff.coor1.y - lut_tool->mouse_stuff.coor2.y);
        box.x = std::min(lut_tool->mouse_stuff.coor1.x, lut_tool->mouse_stuff.coor2.x);
        box.y = std::min(lut_tool->mouse_stuff.coor1.y, lut_tool->mouse_stuff.coor2.y);

        cv::Mat sel_roi(lut_tool->all_in_one, box);
        cv::imshow("[lut_tool] Selected ROI", sel_roi);
